<div class="col-lg-4">
	...
	?>
		<div class="panel-heading" style="color:red;font-size: 20px "><center><strong>Мои чаты <span class="badge"><?php echo $numme; ?></span></strong></center></div>
		<div class="panel-body">
		<table width="100%" class="table table-striped table-bordered table-hover" id="myChatRoom">
			<thead>
			<th>Имя чат-комнаты</th>
			<th></th>
			</thead>
			<tbody>
			<?php
							...
							<td>
								<?php
									...
									if (mysqli_num_rows($memb)>0){
										?>
										<button type="button" class="btn btn-danger btn-sm delete2" value="<?php echo $myrow['chatroomid']; ?>">Удалять</button>
										<?php
									}
									else{
										?>
										<button type="button" class="btn btn-warning btn-sm leave2" value="<?php echo $myrow['chatroomid']; ?>">Покинуть</button>
										<?php
									}
								?>
							</td>
